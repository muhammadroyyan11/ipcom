...
function fetch_product_data($connect)
{
  ...
  ospos_sales.employee_id
FROM ospos_sales_items
  INNER JOIN ospos_sales
    ON ospos_sales_items.sale_id = ospos_sales.sale_id
  INNER JOIN ospos_people
    ON ospos_sales.employee_id = ospos_people.person_id
GROUP BY ospos_sales.sale_id
HAVING SUM(ospos_sales.sale_status = 0) AND DATE(ospos_sales.sale_time) = DATE('$yesterday')";
	...
	foreach ($result as $row) {
		...
		$output .= '
			<tr>
				...
				<td><center>' . number_format($row["sale_id"]) . '</center></td>
				<td><center>' . $row["sale_time"] . '</center></td>
...
<body>
	...
	<div class="container">
		...
		?>
		<h3 align="center">Laporan Penjualan Indiena Coffee Per Invoice Tanggal <?php echo $yesterday ?></h3>
		<br />
		<form method="post">
