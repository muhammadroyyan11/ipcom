...
class Psydebate extends CI_Controller 
{
	...
	public function index()
	{
		...
		$data['testimonial'] = $this->PsypaperModel->debate();

		$this->load->view('templates/admin/template');
        $this->load->view('psydebate/psydebate_index_view', $data);
	}
		...
		public function sub()
	{
		...
		$data['testimonial'] = $this->PsypaperModel->debatesub();

		$this->load->view('templates/subadmin/template');
        $this->load->view('psydebate/psydebatesub_index_view', $data);
	}
	...
	public function add()
	{
		...
		if(isset($_POST['form1'])) {
		    ...
		    if($valid == 1) 
		    {
		        ...
		        move_uploaded_file( $path_tmp, './files/'.$final_name );
	...
	$data['setting'] = $this->Model_common->get_setting_data();
		...
		$this->load->view('templates/admin/template');
        $this->load->view('psydebate/psydebate_edit_view', $data);
	}
	...
	$data['setting'] = $this->Model_common->get_setting_data();
		...
		$this->load->view('templates/subadmin/template');
        $this->load->view('psydebate/psydebatesub_edit_view', $data);
	}
    ...
    //     if($data['testimonial']) {
    //         unlink('./files/'.$data['testimonial']['photo']);
    //     }

        $this->PsypaperModel->delete($id);
        $success = 'Testimonial is deleted successfully';
		$this->session->set_flashdata('success',$success);
        redirect(base_url().'Psydebate');
    }
...
public function updatedatadaring2(){
      $result = $this->PsypaperModel->update_data_daring2();
      ...
      redirect('Psydebate/sub', 'refresh');
    }
